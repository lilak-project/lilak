...
ClassImp(LKPulseExtractor);

LKPulseExtractor::LKPulseExtractor(const char* name, const char *path)
{
    fName = name;
    fPath = path;
    Init();
...
void LKPulseExtractor::Clear(Option_t *option)
{
...
void LKPulseExtractor::Print(Option_t *option) const
{
}

void LKPulseExtractor::SetUsePulseFunction(bool value)
{
    if (fPulseFunction==nullptr)
        fPulseFunction = new LKPulse();
    fUsePulseFunction = value;
    fPulseFunction -> SetUsePulseFunction(value);
}

void LKPulseExtractor::SetPulseFunctionParameters(double baseline, double peak, double t0, double alpha, double tau)
{
    if (fPulseFunction==nullptr)
        fPulseFunction = new LKPulse();
    fUsePulseFunction = true;
    fPulseFunction -> SetPulseFunctionParameters(baseline, peak, t0, alpha, tau);
    fPulseFunction -> SetUsePulseFunction(true);
}

void LKPulseExtractor::SetPulseFunctionRange(double tbMin, double tbMax)
{
    if (fPulseFunction==nullptr)
        fPulseFunction = new LKPulse();
    fUsePulseFunction = true;
    fPulseFunction -> SetPulseFunctionRange(tbMin, tbMax);
    fPulseFunction -> SetUsePulseFunction(true);
}

void LKPulseExtractor::FixPulseFunctionAlpha(bool value)
{
    if (fPulseFunction==nullptr)
        fPulseFunction = new LKPulse();
    fPulseFunction -> FixPulseFunctionAlpha(value);
}

void LKPulseExtractor::FixPulseFunctionTau(bool value)
{
    if (fPulseFunction==nullptr)
        fPulseFunction = new LKPulse();
    fPulseFunction -> FixPulseFunctionTau(value);
}

TF1* LKPulseExtractor::GetPulseFunction(TString name, bool subtractBaseline)
{
    if (fPulseFunction==nullptr)
        fPulseFunction = new LKPulse();
    return fPulseFunction -> GetPulseFunction(name, subtractBaseline);
}

bool LKPulseExtractor::FitPulseFunction(Option_t *option)
{
    if (fPulseFunction==nullptr)
        fPulseFunction = new LKPulse();

    auto graph = GetReferencePulse();
    auto ok = fPulseFunction -> FitPulseFunction(graph, option);
    fUsePulseFunction = true;
    return ok;
}

void LKPulseExtractor::AddChannel(int *data, int event, int cobo, int asad, int aget, int channel)
{
    fEventID = event;
    fCobo = cobo;
    fAsad = asad;
...
void LKPulseExtractor::AddHist(TH1 *hist, int channelID)
{
    if (hist==nullptr)
        return;

    int buffer[512] = {0};
    int tbRange2 = fTbRange2;
    if (tbRange2>512)
        tbRange2 = 512;
    if (tbRange2>hist->GetNbinsX())
        tbRange2 = hist->GetNbinsX();

    for (auto tb=fTbRange1; tb<tbRange2; ++tb)
        buffer[tb] = hist -> GetBinContent(tb+1);

    AddChannel(buffer, channelID);
}

void LKPulseExtractor::AddChannel(int *data, int channelID)
{
    fPreValue = 0;
    fCurValue = 0;
    fCountPulse = 0;
    ...
    for (auto tb=fRefRange1; tb<fRefRange2; ++tb) {
        if (tb<fTbRange1 || tb>=fTbRange2)
            continue;
        fPedestalPry += fChannelData[tb];
        fCountPedestalPry++;
    }
    for (auto tb=fRefRange3; tb<fRefRange4; ++tb) {
        if (tb<fTbRange1 || tb>=fTbRange2)
            continue;
        fPedestalPry += fChannelData[tb];
        fCountPedestalPry++;
    ...
    if (fCountPedestalPry>0)
        fPedestalPry = fPedestalPry / fCountPedestalPry;
    if (fFixPedestal>-10000)
        fPedestalPry = fFixPedestal;

    for (auto tb=fTbRange1; tb<fTbRange2; ++tb)
    {
        fCurValue = fChannelData[tb] - fPedestalPry;
    ...
    if (fCountPulse==1 && fCountWidePulse==0 && fMaxValue>fPulseHeightMin && fMaxValue<fPulseHeightMax)
    {
        ...
        if (fFirstPulseTb>fPulseTbMin && fFirstPulseTb<fPulseTbMax)
        {
            ...
            double dyPulse = fChannelData[fTbAtMaxValue] - fPedestal;
            if (dyPulse<=0)
                return;
            fMaxValue = dyPulse;
            ...
            fHistHeightWidth -> Fill(fMaxValue,fWidth,10);
...
void LKPulseExtractor::DumpChannel(Option_t *option)
{
...
TFile* LKPulseExtractor::WriteTree()
{
...
LKDrawing* LKPulseExtractor::DrawChannel()
{
    ...
    auto drawing = new LKDrawing(Form("channel_%s_%d",fName.Data(),fCountHistChannel));
    drawing -> Add(hist);

    if (fIsCollected) {
        double yPeak = fPedestal + fMaxValue;
        double dy = fMaxValue;
        double y1 = fPedestal - dy*0.20;
        double y2 = yPeak + dy*0.20;
        if (y1<=0) y1 = 0;
    ...
    if (fIsCollected) {
        double yPeak = fPedestal + fMaxValue;
        auto lineC = new TLine(fTbAtMaxValue,fPedestal,fTbAtMaxValue,yPeak);
        lineC -> SetLineColor(kBlue);
        lineC -> SetLineStyle(2);
        drawing -> Add(lineC,"same");

        auto lineT = new TLine(0,fPedestal+fThreshold,fTbRange2,fPedestal+fThreshold);
        lineT -> SetLineColor(kGreen);
        lineT -> SetLineStyle(2);
        drawing -> Add(lineT,"same");

        auto line1 = new TLine(0,fPedestal+fPulseHeightMin,fTbRange2,fPedestal+fPulseHeightMin);
        line1 -> SetLineColor(kRed);
        line1 -> SetLineStyle(2);
        drawing -> Add(line1,"same");

        auto line2 = new TLine(0,fPedestal+fPulseHeightMax,fTbRange2,fPedestal+fPulseHeightMax);
        line2 -> SetLineColor(kRed);
        line2 -> SetLineStyle(2);
        drawing -> Add(line2,"same");

        auto lineP = new TLine(0,fPedestal,fTbRange2,fPedestal);
        lineP -> SetLineColor(kViolet);
        lineP -> SetLineStyle(2);
        drawing -> Add(lineP,"same");
    }
...
void LKPulseExtractor::MakeHistAverage()
{
    if (fHistAverage==nullptr)
...
LKDrawing* LKPulseExtractor::DrawAverage()
{
    MakeHistAverage();
    fHistAverage -> SetTitle("Average pulse;time bucket from peak;normalized ADC");

    auto drawing = new LKDrawing(Form("average_%s",fName.Data()));
    drawing -> Add(fHistAverage,"hist","Average pulse");
    ...
    for (auto ratio : {fFloorRatio, 0.25, 0.50, 0.75})
    {
        ...
        auto tt = new TText(60,ratio,Form("%.2f (at y=%.2f)",width,ratio));
        tt -> SetTextFont(132);
        tt -> SetTextSize(0.07);
        tt -> SetTextAlign(12);
        drawing -> Add(tt,"same");
        auto line = new TLine(tbLeading,ratio,tbTrailing,ratio);
...
LKDrawing* LKPulseExtractor::DrawAccumulate()
{
    SetHist(fHistAccumulate);
    fHistAccumulate -> SetTitle("Accumulated pulses;time bucket from peak;normalized ADC");

    auto drawing = new LKDrawing(Form("accumulate_%s",fName.Data()));
    drawing -> SetLogz();
    drawing -> SetCreateLegend();
    fHistAccumulate -> SetMinimum(1);
    drawing -> Add(fHistAccumulate,"colz","Accumulated pulses");
    //fHistAccumulate -> SetTitle(Form("[%s] %d channels",fName.Data(),fCountGoodChannels));

    {
        GetReferencePulse();
        fGraphReferenceM100 -> SetLineColor(kRed);
        drawing -> Add(fGraphReferenceM100,"samepl","Reference pulse");
    }
...
void LKPulseExtractor::MakeAccumulatePY()
{
    ...
    if (fHistAccumulate!=nullptr)
    {
        for (auto tBin=1; tBin<=fTbRange2; ++tBin)
        {
            ...
            fHistArray -> AddAtAndExpand(hist,tBin-1);
        }
    }
}

LKDrawing* LKPulseExtractor::DrawResidual()
{
    auto drawing = new LKDrawing(Form("residual_%s",fName.Data()));
    drawing -> SetLogz();
    ...
    if (fHistAverage!=nullptr && fHistAccumulate!=nullptr)
    {
        int numBinsY = 40;
        double rMax = .20;
        fHistResidual = new TH2D(Form("histResidual_%s",fName.Data()),Form("Residual;%s;%s","time bucket from peak","average - sample"),
        ...
        drawing -> Add(fHistResidual,"colz");
    }

    return drawing;
}

LKDrawing* LKPulseExtractor::DrawReference()
{
    if (fHistReference==nullptr)
        fHistReference = new TH2D(Form("histReference_%s",fName.Data()),"Reference pulse;time bucket from reference;normalized ADC",100,0,50,140,-0.2,1.2);
    ...
    auto drawing = new LKDrawing(Form("reference_%s",fName.Data()));
    drawing -> SetCreateLegend();
    ...
    */
    ...
    drawing -> Add(fHistReference,"","legendx");
    GetReferencePulse();
    drawing -> Add(fGraphReference,"samepl","Reference pulse");

    auto line0 = new TLine(0,0,50,0);
    line0 -> SetLineColor(kRed);
    line0 -> SetLineStyle(2);
    drawing -> Add(line0,"same");

    return drawing;
}

TGraphErrors *LKPulseExtractor::GetReferencePulse(int tbOffsetFromHead, int tbOffsetFromTail)
{
    MakeAccumulatePY();

    if (fCountGoodChannels==0)
    {
        e_error << "No collected pulse channels. Cannot create reference pulse." << endl;
...
TFile* LKPulseExtractor::WriteReferencePulse(int tbOffsetFromHead, int tbOffsetFromTail)
{
    MakeHistAverage();
    if (fHistAverage==nullptr)
    ...
    auto graphReference = GetReferencePulse(tbOffsetFromHead,tbOffsetFromTail);
    if (graphReference==nullptr || graphReference->GetN()==0)
    {
        e_error << "Reference pulse is empty. Skip writing " << fileName << endl;
        file -> Close();
        return (TFile*) nullptr;
    }
    if (fWritePulseFunctionParametersOnly)
    {
        if (fPulseFunction==nullptr)
            fPulseFunction = new LKPulse();
        fPulseFunction -> FitPulseFunction(fGraphReference, "RQ0");
        fUsePulseFunction = true;
    }
    ...
    if ((fUsePulseFunction || fWritePulseFunctionParametersOnly) && fPulseFunction!=nullptr)
    {
        (new TParameter<bool>("usePulseFunction",true)) -> Write();
        (new TParameter<double>("pulseFunctionBaseline",fPulseFunction->GetPulseFunctionBaseline())) -> Write();
        (new TParameter<double>("pulseFunctionPeak",fPulseFunction->GetPulseFunctionPeak())) -> Write();
        (new TParameter<double>("pulseFunctionT0",fPulseFunction->GetPulseFunctionT0())) -> Write();
        (new TParameter<double>("pulseFunctionAlpha",fPulseFunction->GetPulseFunctionAlpha())) -> Write();
        (new TParameter<double>("pulseFunctionTau",fPulseFunction->GetPulseFunctionTau())) -> Write();
        (new TParameter<double>("pulseFunctionTbMin",fPulseFunction->GetPulseFunctionTbMin())) -> Write();
        (new TParameter<double>("pulseFunctionTbMax",fPulseFunction->GetPulseFunctionTbMax())) -> Write();
    }

    return file;
}

TString LKPulseExtractor::WritePulseParameterFile(int tbOffsetFromHead, int tbOffsetFromTail, TString fileName)
{
    auto graphReference = GetReferencePulse(tbOffsetFromHead,tbOffsetFromTail);
    if (graphReference==nullptr || graphReference->GetN()==0)
    {
        e_error << "Reference pulse is empty. Skip writing pulse parameter file." << endl;
        return "";
    }
    ...
    auto fitIsGood = fPulseFunction -> FitPulseFunction(fGraphReference, "RQ0");
    if (!fitIsGood)
    {
        e_error << "Pulse-function fit failed. Skip writing pulse parameter file." << endl;
        return "";
    }
    fUsePulseFunction = true;

    if (fileName.IsNull())
        fileName = Form("%s/pulse_reference_%s.mac",fPath.Data(),fName.Data());
    fileName.ReplaceAll("//","/");

    ofstream file(fileName.Data());
    if (!file.is_open())
    {
        e_error << "Cannot write " << fileName << endl;
        return "";
    }

    file << "# LKPulse parameter file generated by LKPulseExtractor" << endl;
    file << "LKPulse/PulseFunctionPeak " << fPulseFunction->GetPulseFunctionPeak() << " # Normalized peak height." << endl;
    file << "LKPulse/PulseFunctionT0 " << fPulseFunction->GetPulseFunctionT0() << " # (tb) Function start time relative to hit time." << endl;
    file << "LKPulse/PulseFunctionAlpha " << fPulseFunction->GetPulseFunctionAlpha() << " # Shape exponent." << endl;
    file << "LKPulse/PulseFunctionTau " << fPulseFunction->GetPulseFunctionTau() << " # (tb) Decay time scale." << endl;
    file << "LKPulse/PulseFunctionTbMin " << fPulseFunction->GetPulseFunctionTbMin() << " # (tb) Valid lower range." << endl;
    file << "LKPulse/PulseFunctionTbMax " << fPulseFunction->GetPulseFunctionTbMax() << " # (tb) Valid upper range." << endl;
    file << "LKPulse/PulseFunctionErrorScale 0.05 # Relative error for analytic pulse mode." << endl;
    file << "LKPulse/Inverted " << (fInvertChannel ? "true" : "false") << endl;
    file << endl;
    file << "LKPulseExtracted/PulseFunctionBaseline " << fPulseFunction->GetPulseFunctionBaseline() << " # Normalized baseline offset from fit." << endl;
    file << "LKPulseExtracted/NumAnalyzedChannels " << fCountGoodChannels << endl;
    file << "LKPulseExtracted/Threshold " << fThreshold << " # (ADC) Pulse height threshold above pedestal." << endl;
    file << "LKPulseExtracted/HeightMin " << fPulseHeightMin << " # (ADC) Minimum pulse height above pedestal." << endl;
    file << "LKPulseExtracted/HeightMax " << fPulseHeightMax << " # (ADC) Maximum pulse height above pedestal." << endl;
    file << "LKPulseExtracted/TbMin " << fPulseTbMin << " # First threshold-crossing TB cut lower bound." << endl;
    file << "LKPulseExtracted/TbMax " << fPulseTbMax << " # First threshold-crossing TB cut upper bound." << endl;
    file << "LKPulseExtracted/FWHM " << fFWHM << " # (tb) Full width at half maximum." << endl;
    file << "LKPulseExtracted/FloorRatio " << fFloorRatio << endl;
    file << "LKPulseExtracted/Width " << fRefWidth << " # (tb) Width at FloorRatio." << endl;
    file << "LKPulseExtracted/WidthLeading " << fWidthLeading << " # (tb) Leading width from floor to peak." << endl;
    file << "LKPulseExtracted/WidthTrailing " << fWidthTrailing << " # (tb) Trailing width from peak to floor." << endl;
    file << "LKPulseExtracted/PulseRefTbMin " << fPulseRefTbMin << endl;
    file << "LKPulseExtracted/PulseRefTbMax " << fPulseRefTbMax << endl;
    file << "LKPulseExtracted/BackGroundLevel " << fBackGroundLevel << " # (ADC) Pedestal level." << endl;
    file << "LKPulseExtracted/BackGroundError " << fBackGroundError << " # (ADC) Pedestal residual sigma." << endl;
    file << "LKPulseExtracted/FluctuationLevel " << fFluctuationLevel*fYMax << " # (ADC) Pedestal fluctuation level." << endl;

    e_info << "Writting " << fileName << " (" << fCountGoodChannels << " channels)" << endl;
    return fileName;
}

LKDrawing* LKPulseExtractor::DrawWidth()
{
    fHistWidth -> SetTitle("Pulse width;width at threshold (tb);channels");
    SetHist(fHistWidth);
    auto drawing = new LKDrawing(Form("width_%s",fName.Data()));
    drawing -> Add(fHistWidth,"hist","Width");
...
LKDrawing* LKPulseExtractor::DrawHeightWidth()
{
    fHistHeightWidth -> SetTitle("Pulse height vs width;pulse height above pedestal (ADC);width at threshold (tb)");
    SetHist(fHistHeightWidth);
    auto drawing = new LKDrawing(Form("height_width_%s",fName.Data()));
    drawing -> Add(fHistHeightWidth,"colz");

    return drawing;
}

LKDrawing* LKPulseExtractor::DrawHeight()
{
    fHistHeight -> SetTitle("Pulse height;pulse height above pedestal (ADC);channels");
    fHistPedestalPry -> SetTitle("Preliminary pedestal;ADC;channels");
    SetHist(fHistHeight);
    auto drawing = new LKDrawing(Form("height_%s",fName.Data()));
    drawing -> SetCreateLegend();
    drawing -> Add(fHistHeight,"hist","Pulse height");

    fHistPedestalPry -> SetLineColor(kRed);
    drawing -> Add(fHistPedestalPry,"same","Preliminary pedestal");
...
LKDrawing* LKPulseExtractor::DrawPulseTb()
{
    fHistPulseTb -> SetTitle("Pulse time;first threshold-crossing TB;channels");
    auto drawing = new LKDrawing(Form("pulse_tb_%s",fName.Data()));
    drawing -> Add(fHistPulseTb,"hist","Pulse time");
...
LKDrawing* LKPulseExtractor::DrawPedestal()
{
    fHistPedestal -> SetTitle("Pedestal;pedestal ADC;channels");
    SetHist(fHistPedestal);
    auto drawing = new LKDrawing(Form("pedestal_%s",fName.Data()));
    drawing -> Add(fHistPedestal,"hist","Pedestal");
...
double LKPulseExtractor::FullWidthRatioMaximum(TH1D *hist, double ratioFromMax, double numSplitBin, double &tbLeading, double &tbTrailing, double &error)
{
...
void LKPulseExtractor::SetCvs(TCanvas *cvs)
{
    cvs -> SetLeftMargin(0.1);
    cvs -> SetRightMargin(0.15);
}

void LKPulseExtractor::SetHist(TH1 *hist)
{
...
void LKPulseExtractor::Draw(Option_t *option)
{
    if (fTop==nullptr)
        fTop = new LKDrawingGroup(Form("pulse_extractor_%s",fName.Data()));
    else
        fTop -> Clear();

    //fTop -> SetCanvasDivision(3,4);
    //fTop -> AddDrawing(DrawChannel());
    fTop -> AddDrawing(DrawAverage());
    fTop -> AddDrawing(DrawAccumulate());
    fTop -> AddDrawing(DrawWidth());
    fTop -> AddDrawing(DrawHeight());
    fTop -> AddDrawing(DrawPulseTb());
    fTop -> AddDrawing(DrawPedestal());
    fTop -> AddDrawing(DrawResidual());
    fTop -> AddDrawing(DrawReference());
    fTop -> AddDrawing(DrawHeightWidth());
