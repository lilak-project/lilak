#ifndef LKPULSEEXTRACTOR_HH
#define LKPULSEEXTRACTOR_HH
...
/**
 * @brief LKPulseExtractor find channels containing single pulse and create reference pulse from their average.
 * The LKPulseExtractor find the reference pulse from the group of channels those are likely to have similar waveform with same width.
 *
 ...
 * @endcode
 *
 * Example of using LKPulseExtractor:
 *
 * @code{.cpp}
 *  {
 *       auto ana = new LKPulseExtractor("MyExperiment");
 ...
 * @endcode
 */
class LKPulseExtractor : public TObject
{
    public:
        //LKPulseExtractor(const char* name, const char *path=".");
        LKPulseExtractor(const char* name, const char *path=".");

        LKPulseExtractor() : LKPulseExtractor(0) { ; }
        virtual ~LKPulseExtractor() { ; }
        ...
        void AddHist(TH1 *hist, int channelID=-1);
        void AddHist(TH1D *hist, int channelID=-1) { AddHist((TH1*) hist, channelID); }
        ...
        LKDrawing* DrawChannel();
        void MakeAccumulatePY();
        void MakeHistAverage();
        LKDrawing* DrawAverage();
        LKDrawing* DrawAccumulate();
        LKDrawing* DrawWidth();
        LKDrawing* DrawHeight();
        LKDrawing* DrawPulseTb();
        LKDrawing* DrawPedestal();
        LKDrawing* DrawResidual();
        LKDrawing* DrawReference();
        LKDrawing* DrawHeightWidth();
        LKDrawingGroup* GetTop() { return fTop; }
        ...
        TString WritePulseParameterFile(int tbOffsetFromHead=0, int tbOffsetFromTail=0, TString fileName="");
        ...
        bool FitPulseFunction(Option_t *option="RQ0");
        void SetWritePulseFunctionParametersOnly(bool value=true) { fWritePulseFunctionParametersOnly = value; }
    ...
    public:
        ...
        LKPulse*      fPulseFunction = nullptr; ///< Analytic pulse function used to fit or export the reference pulse.
        bool          fUsePulseFunction = false; ///< If true, WriteReferencePulse also stores analytic pulse-function parameters.
        bool          fWritePulseFunctionParametersOnly = false; ///< If true, fit extracted reference pulse and write only LKPulse function parameters.
