...
class LKVector3 : public TVector3
{
    public:
        enum Axis : int {
            kNon = 0,
        ...
        LKVector3(Axis referenceAxis)
            :TVector3(), fReferenceAxis(referenceAxis) {}

        LKVector3(const LKVector3 &p)
            :TVector3(p), fReferenceAxis(p.fReferenceAxis) {}

        LKVector3(const TVector3 &p, Axis referenceAxis = kNon)
            :TVector3(p), fReferenceAxis(referenceAxis) {}

        LKVector3(Double_t x, Double_t y, Double_t z, Axis referenceAxis = kNon)
            :TVector3(x,y,z), fReferenceAxis(referenceAxis) {}

        LKVector3(Axis referenceAxis, Double_t i, Double_t j, Double_t k)
            :fReferenceAxis(referenceAxis) { SetIJK(i,j,k); }

        LKVector3(Axis referenceAxis, TVector3 ijk)
            :fReferenceAxis(referenceAxis) { SetIJK(ijk.X(),ijk.Y(),ijk.Z()); }

        virtual ~LKVector3() {}

        inline LKVector3 operator - () const {
            return LKVector3(-X(), -Y(), -Z(), fReferenceAxis);
        }

        inline LKVector3 & operator = (const LKVector3 & p) {
            SetX(p.X());
            SetY(p.Y());
            SetZ(p.Z());
        ...
        //Double_t Angle2(const LKVector3 &q, TVector3 ref) const;

    private:
        Axis fReferenceAxis = kNon;
        //Double_t fSortBy = 0;

        ClassDef(LKVector3, 1)
};
