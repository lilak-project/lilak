...
class lilakcc:
    ...
    def __init__(self, input_lines=''):
        ...
        self.list_complete = ["path", "class", "idata", "odata", "kdata", "include", "private", "protected", "public", "enum"]
        self.list_components = ["comment", "inherit", "gname", "lname", "pname", "bname", "persis", "setter", "getter", "init", "clear", "source", "print", "copy", "bclass"]
        if len(input_lines)!=0: self.add(input_lines)
        self.lf_keywords = [
            # name      , main, class, io, par, title                    , comment
            ["comment"  ,    1,     1,  1,   1, "Add comment"            , "// This is comment"                  , "",],
            ["path"     ,    1,     0,  0,   0, "Set output path"        , "+path /path/to/output"               , "Path to create output file",],
            ["class"    ,    1,     9,  0,   0, "Set class name"         , "+calss LKMyClass"                    , "Class name",],
            ["idata"    ,    1,     0,  0,   0, "Add input data cont."   , "+idata fChannelArray\n+bclass GETChannel 200\n+bname  RawData", "Input data",],
            ["odata"    ,    1,     0,  0,   0, "Add output data cont."  , "+odata fHitArray\n+bclass LKHit 200\n+bname  Hit", "Output data",],
            ["kdata"    ,    1,     0,  0,   0, "Add keep data cont."    , "+kdata  fEventHeader\n+bclass TTEventHeader\n+bname  EventHeader", "Keep data",],
            ["include"  ,    1,     0,  0,   0, "Inlude header"          , "+include TFile.h"                    , "Included header",],
            ["private"  ,    1,     0,  9,   9, "Start private member"   , "+private eType fType[3][4][4][68]"   , "Create private method or parameter",],
            ["protected",    1,     0,  9,   9, "Start protected member" , "+protected TFile* fInputFile"        , "Create protected method or parameter",],
            ["public"   ,    1,     0,  9,   9, "Start public member"    , "+publuc  TString fFileName = /home/path/to/data/data.root", "Create public method or parameter",],
            ["enum"     ,    0,     0,  0,   0, "Add enum class"         , "+enum eDetLoc\n kNon\n kLeft // 0\n kRight // 1\n", "",],
            ["inherit"  ,    0,     1,  0,   0, "Inherit class"          , "inherit LKPadInteractive.h"          , "Inheritance of class ex: public TNamed",],
            #["setter"   ,    0,     0,  0,   1, "Add Set___() method"    , ""                                    , "Set setter",],
            #["getter"   ,    0,     0,  0,   1, "Add Get___() method"    , ""                                    , "Set getter",],
            #["init"     ,    0,     0,  0,   1, "Add Init() content"     , ""                                    , "Set content to be included in the Constructor of LKTask",],
            #["clear"    ,    0,     0,  0,   1, "Add Clear() content"    , ""                                    , "Set content to be included in the Init() method of LKTask",],
            #["source"   ,    0,     0,  0,   1, "Add constructor content", ""                                    , "Set content to be included in the Clear() method of LKTask",],
            #["print"    ,    0,     0,  0,   1, "Add Print() content"    , ""                                    , "Set content to be included in the Print() method of LKTask",],
            #["copy"     ,    0,     0,  0,   1, "Add Copy() content"     , ""                                    , "Set content to be included in the Copy() method of LKTask",],
            #["gname"    ,    0,     0,  1,   1, "Set global name"        , ""                                    , "Set global data container array name",],
            #["lname"    ,    0,     0,  1,   1, "Set local name"         , ""                                    , "Set local data container array name",],
            #["bname"    ,    0,     0,  1,   0, "Set branch name"        , ""                                    , "Set parameter name to be registered or registered as branch name",],
            #["pname"    ,    0,     0,  1,   1, "Set parameter name"     , ""                                    , "Set local single data container name",],
            #["persis"   ,    0,     0,  1,   1, "Set persistency"        , ""                                    , "Set persistency of branch (True or False). Default is True",],
            #["bclass"   ,    0,     0,  1,   0, ""                       , ""                                    , "Class of the branch ex) [class_name] [size].\nIf [size] is given, branch is created as TClonesArray.\n If [size] is not given, branch is created with [class_name].",],
        ]
    ...
    def add(self, input_lines):
        ...
        for line in list_line:
            if len(line)==0:
                ...
                if ltype=='':
                    ltype=='public'
                if ltype in self.list_complete:
                ...
                elif ltype in self.list_components:
                    group.append([line[0],ltype,content])
            else:
        ...
        for group in group_list:
            ...
            for i2 in range(len(group_new)):
                ...
                if ltype0 in self.list_complete:
                    break
